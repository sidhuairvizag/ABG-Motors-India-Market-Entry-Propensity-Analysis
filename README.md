...
### Raw data (inputs — do not overwrite)

- `JPN Data.xlsx` — Japanese training market. 40,000 rows. Has `PURCHASE`.
- `IN_Data.xlsx` — Indian scoring market. 70,000 rows. No `PURCHASE`. Has `DT_MAINT`.

### Processed outputs (generated by the notebook)

- `Indian_Scored_Customers.csv` — scored India file: probabilities and 0/1 flags for logistic and XGBoost, unadjusted and income-aligned.
- `Logistic_Coefficients.csv` — log-odds and odds ratios for the primary model.
- `Model_Performance_Comparison.csv` — holdout metrics for logistic vs XGBoost.

### Analysis and presentation

- `ABG-Motors-India-Market-Entry-Propensity-Analysis.ipynb`
- `ABG Motors – India Market Entry Decision.twb`
- Dashboard 1 — India market-entry decision
- Dashboard 2 — model performance and drivers
- Dashboard 3 — Indian opportunity and CRM
- Dashboard 4 — Japan training market: what drives purchase
